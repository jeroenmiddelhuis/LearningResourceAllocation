	...
	def q_learning(self, total_episodes, plot_every=100):
		...
		for episode in range(total_episodes + 1):
			...
			while True:
				...
				max_value_next_state = max([self.qtable[new_state[0].task_type][self.simulator.resources.index(resource)][self.workloads.index(self.get_workload(new_state[1], resource))] 
...
if __name__ == '__main__':
	...
	for config_type in ['complete_parallel']: #['slow_server', 'n_system', 'high_utilization', 'low_utilization', 'parallel', 'down_stream']:# ['complete', 'complete_reversed', 
		running_time = 5000
		num_cpu = 1
		load_model = False
		...
		with open(f"./RLRAM_models/{config_type}.json", 'w') as json_file: 
