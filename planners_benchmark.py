...
class DDQNPlanner(Planner):
    ...
    def create_model(self):
        ...
        self.model = keras.models.Sequential([
                keras.layers.Flatten(input_shape=(len(self.simulator.input),)),
                ...
                keras.layers.BatchNormalization(),
                keras.layers.ReLU(),
                keras.layers.Dense(len(self.simulator.output))
            ])
        self.model.load_weights(os.getcwd() + f"/tmp_DDQN/DDQN_{self.model_name}/best_weigths.h5")
    ...
    def plan(self, available_resources, unassigned_tasks, resource_pool):
        state = self.get_state_DDQN()    
        
        # We have to go to the next state if the action take is infeasible
        # Otherwise, the state and action will be the same
