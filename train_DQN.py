...
from bpo_env_DQN import BPOEnv
from simulator_RLRAM import Simulator
import time
import gc

class DDQN_train:
    def __init__(self, running_time, time_steps, learning_rate, reward_function, gamma, epsilon, batch_size, config_type):
        ...
        self.log_dir = f"./tmp_DDQN/{config_type}/"
        self.env = BPOEnv(running_time=running_time, config_type=config_type, 
                    reward_function=reward_function, postpone_penalty=0,
                    write_to=self.log_dir)
        ...
        available_resources = self.env.simulator.resources

        self.observation_space = self.env.simulator.input
        self.action_space = self.env.simulator.output
    ...
    def epsilon_greedy_policy(self, state, epsilon=0, type_of_epsilon_exp="random"):
        """
        ...
        if np.random.rand() < epsilon:
            if type_of_epsilon_exp == "random":
                ...
                next_state, reward, isTerminated, _, _ = self.env.step(action)
                return next_state, reward, action, "r", isTerminated
            ...
            self.q_values_mean
            self.q_values_counter
            self.q_values_mean += np.mean(Q_values[0])
            self.q_values_counter += 1
            action = np.argmax(Q_values[0])
            next_state, reward, isTerminated, _, _ = self.env.step(action)
            return next_state, reward, action, "q", isTerminated
    ...
    def play_one_step_and_collect_memory(self, state, epsilon, type_of_epsilon_exp="random"):
        next_state, reward, action, action_type, isTerminated = self.epsilon_greedy_policy(state,
                                                                        epsilon, type_of_epsilon_exp)
        self.replay_memory.append((state, action, reward, next_state))
        return next_state, reward, action, action_type, isTerminated
    ...
    def main(self):
        ...
        while total_steps < time_steps:
            ...
            while isTerminated == False:                
                ...
                obs, reward, action, action_type, isTerminated = self.play_one_step_and_collect_memory(obs, epsilon, type_of_epsilon_exp)

                if episode_nmb > nmb_of_episodes_before_training and step % 4 == 0:
                    episode_loss += self.training_step(self.batch_size)
                if episode_nmb > nmb_of_episodes_before_training and step % 10000 == 0:
            ...
            print("Episode: {}, time_steps: {}, loss: {}, rewards: {}, q_value_mean: {} actions: {} eps: {:.3f} time: {}".format(episode_nmb,
                                                                                                                total_steps,
                                                                                                                episode_loss/nmb_of_iterations_per_episode,
                                                                                                                train_episode_reward_sum,
                                                                                                                self.q_values_mean/self.q_values_counter,
        ...
        self.model.save(f'./tmp_DDQN/DDQN_{self.config_type}/')
        self.model.save_weights(f'./tmp_DDQN/DDQN_{self.config_type}/best_weigths.h5')
        with open(f'./tmp_DDQN/DDQN_{self.config_type}' + '/report.txt', 'w') as file:
        ...
        #     test_episode_reward_sum = 0
        #     for step in range(nmb_of_iterations_per_episode):
        #         state, reward, action, action_type = self.epsilon_greedy_policy(state)
        #         state = state.to_numpy()
        #         test_episode_reward_sum += reward
...
if __name__ == '__main__':
    ...
    reward_function = 'case_completion'
    gamma = 1
    epsilon = 1 # epsilon linearly annealed from 1 to 0.1 over the first 10% of episodes
    batch_size = 32 
    config_type = 'slow_server'
    
    ddqn_train = DDQN_train(running_time, time_steps, learning_rate, reward_function, gamma, epsilon, batch_size, config_type)
    ddqn_train.main()