...
class EvalPolicyCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls > 1 and (self.n_calls - 1) % self.check_freq == 0:
            ...
            for epoch in range(self.nr_evaluations):
                ...
                while isTerminated == False:
                    action = self.model.predict(state, action_masks=self.eval_env.action_masks())[0]
                    state, reward, isTerminated, _, _ = self.eval_env.step(action)

                mean_cycle_times.append(self.eval_env.simulator.sumx / self.eval_env.simulator.sumw)
